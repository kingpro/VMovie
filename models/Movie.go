...
func (m *MovieInfo) GetRandList(pagesize int64) []*MovieInfo {
	list := make([]*MovieInfo, 0)
	o := orm.NewOrm()
	_, _ = o.Raw("SELECT * FROM movie_info AS t1 JOIN (SELECT ROUND(RAND() * ((SELECT MAX(Id) FROM movie_info)-(SELECT MIN(Id) FROM movie_info)) + (SELECT MIN(Id) FROM movie_info)) AS Id) AS t2 WHERE t1.Id >= t2.Id ORDER BY t1.Id LIMIT 10").QueryRows(&list)
	return list
}
